            ...
            <div class="bg-white rounded-2xl shadow p-6 mb-10 transition hover:shadow-xl">
                ...
                <div class="flex items-center mb-4">
                    <div class="w-11 h-11 bg-blue-600 text-white font-bold rounded-full flex items-center justify-center text-lg">
                        {{ strtoupper(substr($publicacion->title, 0, 1)) }}
                    </div>
                    <div class="ml-4">
                        <h3 class="text-xl font-semibold text-gray-900">{{ $publicacion->title }}</h3>
                        <p class="text-sm text-gray-500">📅 {{ $publicacion->created_at->format('d/m/Y') }}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <img src="{{ asset('storage/' . $img) }}"
                         ...
                         @click="fileUrl='{{ asset('storage/' . $img) }}'; isPdf=false; show=true" />
                    @endforeach
                </div>
                @endif
                ...
                <div class="space-y-3 mb-4">
                    ...
                    <div class="flex items-center justify-between bg-gray-100 p-3 rounded border">
                        <div class="flex items-center gap-2 text-gray-800 truncate">
                            ...
                            <span class="text-sm truncate w-52 md:w-64">{{ basename($pdf) }}</span>
                        </div>
                        <button @click="fileUrl='{{ asset('storage/' . $pdf) }}'; isPdf=true; show=true"
                ...
                <div class="relative inline-block group mb-4">
                    ...
                    <div class="absolute left-0 hidden group-hover:block bg-white border rounded-lg shadow-lg mt-2 z-20 p-3 w-48">
                        ...
                        <a href="https://api.whatsapp.com/send?text={{ $text }}%20{{ $shareUrl }}"
                        ...
                        <a href="mailto:?subject={{ $text }}&body={{ $shareUrl }}"
                ...
                <div class="flex justify-end gap-6 text-sm">
                    <a href="{{ route('publicaciones.edit', $publicacion) }}" class="text-yellow-600 hover:underline">✏️ Editar</a>
